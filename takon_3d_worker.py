...
def run_process_mode(job_id, video_url, work_dir):
    ...
    # 2. Extract Frames using ffmpeg (Reduce to 2 fps to lower total image count for RAM stability)
    print("🎞️ Extracting frames with ffmpeg (2 fps, 720p)...", flush=True)
    run_command(f"ffmpeg -i {video_path} -q:v 2 -vf \"fps=2,scale=-1:720\" {images_dir}/frame_%04d.jpg")
    ...
    if not success: 
        print(f"⚠️ GLOMAP Script failed, attempting fallback with ns-process-data images (Lower target)...", flush=True)
        # Reduce target frames even more for fallback stability
        cmd_fallback = f"ns-process-data images --data {images_dir} --output-dir {output_dir} --num-frames-target 80"
